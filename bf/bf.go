...
func (bloom bloomfilter) insert(key string) {
	...
	for j := 0; j < bloom.Hash_size; j++ {
		// fmt.Println("j, hash_seed", j, bloom.Hash_seeds[j])
		bitval := int(murmur3.Sum32WithSeed([]byte(key), bloom.Hash_seeds[j])) % bloom.Bitsize
		// fmt.Println("bit val", bitval)
		bloom.Bitvec.setBitAt(bitval)
	}
}
...
func (bloom bloomfilter) query(key string) string {
	...
	// fmt.Println("key", key)
	// fmt.Println("bloom.Bitvec", bloom.Bitvec)

	for j := 0; j < bloom.Hash_size; j++ {
		// fmt.Println("j, hash_seed", j, bloom.Hash_seeds[j])
		bitval := int(murmur3.Sum32WithSeed([]byte(key), bloom.Hash_seeds[j])) % bloom.Bitsize
		// fmt.Println("bit val", bitval)
		sum += bloom.Bitvec.getBitAt(bitval)
	}

	if int(sum) == bloom.Hash_size {
		return "Y"
	}
...
func main() {
	...
	switch args[0] {
		case "build":
			...
			scanner := bufio.NewScanner(file)
			for scanner.Scan() {
				bloom.insert(strings.TrimSpace(scanner.Text()))
			}
		...
		case "query":
			...
			for scanner.Scan() {
				key := strings.TrimSpace(scanner.Text())
				fmt.Println(key + ":" + bloom.query(key))
			}
